...
const Contact = () => {
  ...
  return (
    <section id="contact" className="bg-contact relative" style={{ padding: "clamp(8rem,18vh,16rem) 0 clamp(5rem,10vh,8rem)" }}>
      <div className="glow" style={{ width: "600px", height: "600px", top: "25%", left: "30%", background: "radial-gradient(circle,rgba(59,130,246,0.05) 0%,transparent 55%)" }} />
      ...
      <div className="wn relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-16">
          <span className="lbl block mb-6">Let&apos;s Connect</span>
          <h2 className="d-lg text-white mb-1" style={{ lineHeight: 1.05 }}>Let&apos;s</h2>
          <h2 className="d-lg" style={{ color: "var(--blue)", lineHeight: 1.05 }}>build.</h2>
          <p className="b-sm mt-6" style={{ maxWidth: "380px" }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.1 }}>
            ...
            <button
              ...
              className="a-link w-full justify-center py-4"
              style={{ background: "var(--blue)", color: "white", fontSize: "0.7rem", letterSpacing: "0.12em", fontWeight: 600, border: "none", cursor: "none" }}
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.15 }} className="flex flex-col justify-between">
            <div className="space-y-7">
              {[
                ...
                <div key={item.label}>
                  <p className="lbl mb-1.5">{item.label}</p>
                  <a href={item.href} target="_blank" rel="noopener noreferrer" className="a-link" style={{ fontSize: "0.85rem" }}>
                    {item.value}
            ...
            <div className="flex items-center gap-5 mt-12 lg:mt-0">
              {[
                ...
                <a key={s.l} href={s.href} target="_blank" rel="noopener noreferrer" className="transition-colors duration-300 cursor-none" style={{ color: "var(--text-faint)", fontSize: "1rem" }} onMouseEnter={(e) => (e.currentTarget.style.color = "var(--blue-l)")} onMouseLeave={(e) => (e.currentTarget.style.color = "var(--text-faint)")} aria-label={s.l}>
                  {s.icon}
                </a>
              ))}
            </div>
